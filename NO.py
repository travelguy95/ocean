...
def load_darcy_flow_small(n_train, n_tests,
    ...
    path = Path("").resolve().parent.joinpath('/content/drive/MyDrive/pramya')
...
def load_darcy_pt(data_path,
    ...
    data = torch.load(Path(data_path).joinpath(f'480K_wet_dry_training.pt').as_posix())
    ...
    data = torch.load(Path(data_path).joinpath(f'480K_wet_dry_testing.pt').as_posix())
